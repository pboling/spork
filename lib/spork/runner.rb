...
module Spork
  ...
  class Runner
    ...
    def initialize(args, output, error)
      ...
      opt.on("--test_env_number TEST_ENV_NUMBER") {|number| @options[:test_env_number] = number }
    ...
    def run
      ...
      when options[:diagnose]
        ...
        server_options = {:port => @options[:port] || test_framework.default_port, :run_strategy => run_strategy, :quiet => @options[:quiet]}
        if !@options[:test_env_number].nil?
          ENV['TEST_ENV_NUMBER'] = @options[:test_env_number].to_s
          server_options.merge!(:test_env_number => @options[:test_env_number])
        end
        Spork::Server.run(server_options)
        return true
      end
    end
